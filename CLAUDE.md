...
- **release.yml** — Runs on push to main. Derives the next patch version from the latest git tag, injects it into `src/manifest.json` at build time (no commit back to repo), tags, creates a GitHub Release with `.zip` (Chrome) and `.xpi` (Firefox) assets, and auto-publishes to the Chrome Web Store. The version in `package.json` and `src/manifest.json` on disk is a dev placeholder — the release version comes from git tags.
